...
| 29 | **עטיפת side (square/tight) עדיין על נתיב ה‑Row הישן** (`getFloatsFromParagraph`+IntrinsicHeight), לא band‑reflow אמיתי מבוסס `lineExtent`. השכבה הממוקמת החדשה מטפלת רק ב‑topAndBottom + front (wrapNone/inFront); behindText נשאר רקע‑עמוד. `lineExtent`/`nextUsableY` מוכנים ונבדקים אך לא מחווטים לרינדור side עדיין | band‑reflow דורש מודד רוחב‑משתנה פר‑שורה + RenderObject ייעודי; נדחה כדי לא לשבור parity (7 עמ') בנתיב הקיים. side נשאר כפי שהיה (ללא רגרסיה) | H | בינונית |
