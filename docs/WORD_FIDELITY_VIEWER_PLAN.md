...
- **STYLEREF (§K.3)** — תלוי‑עימוד כמו PAGE: ב‑[paginator.dart](../packages/docx_file_viewer/lib/src/pagination/paginator.dart) `_prepareStyleRefs` סורק גוף+כל וריאנטי כותרת/תחתית ל‑`DocxStyleRef`, ובונה `styleId→שם‑סגנון מנורמל` מ‑`styles.xml`. בזמן מילוי עמוד `_recordStyleRef` עוקב אחר טקסט הפסקה האחרונה/ראשונה לכל סגנון נעקב (carry‑over בין עמודים); ב‑`_closePage` נשמרים `PageModel.styleRefLast/styleRefFirst`. ברירת‑מחדל=אחרון‑עד‑סוף‑העמוד, `\l`=ראשון‑בעמוד. התאמה לפי **שם‑סגנון** (גם עברי) או styleId מנורמל (`"Heading 1"`↔`Heading1`). [field_substitution.dart](../packages/docx_file_viewer/lib/src/pagination/field_substitution.dart) פותר `DocxStyleRef` מ‑`PageContext` (fallback ל‑cachedText).
...
- **סמלים (§K.5)** — [symbol_map.dart](../packages/docx_file_viewer/lib/src/layout/symbol_map.dart) ממפה `w:sym` (Wingdings/Symbol/Webdings) ל‑Unicode שקול (✓✗☑☺ חצים + יוונית/מתמטיקה מלאה של Symbol). תו ממופה→פונט גוף (נראה גם בלי הפונט המקורי); לא‑ממופה→הפונט המקורי + fontFamilyFallback. `DocxSymbol` עתה מרונדר ב‑`buildInlineSpans` (קודם נשר).
