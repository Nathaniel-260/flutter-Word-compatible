...
| טקסט | BiDi מ‑`w:bidi`/`w:rtl` + פיצול כתב ascii/cs | ✅ `w:bidi`/`w:rtl`+szCs/bCs/iCs נקראים (A); יישור BiDi מדויק (טבלת C.4) מחווט; **פיצול כתב ascii/cs מומש (L)** — ריצה מעורבת עברית+לטינית מתפצלת לפי כתב, כל קטע עם הפונט/גודל/משקל הנכון | A,C,L |
...
| הערות | שוליים/סיום על העמוד | ✅ **הערות שוליים בתחתית העמוד** (אזור שמור שגדל כלפי מעלה ומקטין את הגוף; הערה דוחפת שורה לעמוד הבא; separator ⅓‑רוחב; מספור תלוי‑עימוד decimal/roman/letter/**גימטריה** + restart continuous/eachSect/eachPage; ה‑ref בטקסט וההערה מציגים אותו מספר). **הערות סיום** זורמות בסוף המסמך (docEnd) עם מספור רציף. Dialog נשאר ל‑continuous | J |
| שדות | PAGE/NUMPAGES/SECTIONPAGES/PAGEREF | ✅ פענוח+החלפה; פורמטים decimal/roman/letter + **hebrew1/hebrew2** (E) | —,E |
| שדות | STYLEREF/TOC חי/קישורים פנימיים | ✅ **STYLEREF** תלוי‑עימוד (כותרת רצה — אחרון/`\l` ראשון בעמוד, carry‑over; התאמת שם‑סגנון מ‑styles.xml + styleId מנורמל); **TOC** מורחב לפסקאות‑מטמון (leader + PAGEREF חי); **קישורים פנימיים** (`#anchor`→`DocxViewController.jumpToBookmark`/גלילה לעמוד), **חיצוניים** `config.onOpenLink`; **HYPERLINK כשדה**; **SDT** שקוף (dropdown=נבחר, checkbox) | K |
| פונטים | מוטמעים (כולל obfuscated) | ✅ קיים | — |
| פונטים | תחליפים מטריים/fallback לפי כתב/טעינה עצלה | ✅ **`FontResolver`** (שם‑Word→משפחה זמינה: override‑משתמש→מוטמע/מערכת בשם זהה→טבלת תחליפים מטריים Calibri→Carlito, Arial→Arimo, Times→Tinos, David→David Libre, Narkisim→Frank Ruhl Libre…→שם כפי שהוא); **fontFamilyFallback פר‑כתב** (עברית/ערבית: David Libre/Frank Ruhl/Noto לפני ה‑fallbacks של ה‑host); **פיצול כתב** ב‑`resolveRunSegments` (לטינית=ascii/sz/b/i, מורכב=cs/szCs/bCs/iCs); **טעינה עצלה** של מוטמעים (רק אם בשימוש). `DocxViewConfig.fontSubstitutions` | L |
| סמלים | `w:sym` (Wingdings/Symbol) | ✅ מיפוי תו→Unicode בינלאומי (checkbox/smiley/חצים + יוונית/מתמטיקה של Symbol); תו לא‑ממופה → הפונט המקורי + fallback | K |
| נוסחאות | OMML | ✅ מחוץ להיקף (placeholder — טקסט לינארי `m:t`) | K |
...
| 32 | עטיפת side היא **פר‑פסקה** (ה‑float עוטף את טקסט פסקת‑העוגן בלבד). float שגבוה מפסקת‑העוגן לא מאלץ את הפסקאות הבאות לעטוף סביבו (Word כן) — דורש exclusion ברמת‑עמוד (RenderObject של גוף‑העמוד). strut (exact/atLeast) **כן** מושחל כעת לנתיב העטיפה (measure+render). נותרו: first‑line indent ו‑`vFrom`/`vAlign` (מיקום אנכי של ה‑float יחסית לעמוד/שוליים) לא מיושמים בעטיפה הפר‑פסקתית — הצמדה לראש הפסקה (`vOffsetPx`); הבחירה (`SelectableText`) הופכת ל‑`RichText` בפסקת‑float | המקרה הנפוץ (תמונה + טקסט צמוד בפסקתها) מטופל; חוצת‑פסקאות נדיר ויקר. **נחקר ונדחה מודעת (2026‑06‑18):** המודל הנוכחי (§8.2 #29) מנפח את פסקת‑העוגן לגובה ה‑float המלא (`_floatsHeight=max(טקסט,תחתית‑float)`), כך ש‑float לעולם אינו גולש לפסקה הבאה — עטיפה חוצת‑פסקאות אמיתית דורשת **ניתוק גובה ה‑float מגובה הפסקה** + **העברת ציור ה‑side floats לשכבת‑העמוד** (paged) + שמירת מצב continuous (שני נתיבים) + שכתוב ~5 קבצי בדיקה שמקבעים את §8.2 #29 — שכתוב רחב‑היקף ורגיש‑parity, לא תוספה מוכלת. תשתית ההפצה (accumulator פר‑עמוד + מסנן חפיפה) נבדקה כ‑PoC והוסרה | H | נמוכה |
...
| 37 | **גובה אזור-ההערות נמדד מתוכן ההערה ללא מספר-ההערה המוביל.** ה‑Paginator מודד את בלוקי ההערה ברוחב‑התוכן המלא ושומר רצועה בתחתית; הרינדור מזריק את המספר כ‑superscript לראש הפסקה הראשונה. למספר (1–2 תווים superscript) השפעה זניחה על מספר השורות — פער מדידה≡רינדור אפשרי רק כשהשורה הראשונה של הערה מלאה עד ~2 תווים מהקצה (נדיר). הרצועה בתחתית ה‑clip‑area, כך שחריגה של שורה בודדת נחתכת בשקט ולא דוחפת גוף | מדידה מדויקת דורשת לדעת את רוחב המספר לפני הקצאת המספר (תלות מעגלית: הגובה נדרש לבדיקת ההתאמה לפני שהמספר ידוע). הערות קצרות במסמכי קודש → השפעה אפסית | J | נמוכה |
| 38 | **separator ברירת‑מחדל בלבד** — קו ⅓‑רוחב בקצה המוביל. separator מותאם מ‑footnotes.xml (פסקאות `w:type="separator"/"continuationSeparator"`, id −1/0) לא מכובד — ה‑reader משמיט את ה‑`w:type` ואוסף את כל ה‑`w:footnote` (כולל separators) לרשימה אחת; ההפרדה מתבצעת לפי "מופנה‑או‑לא" (separators בעלי id≤0 לעולם לא מופנים, ולכן אף פעם לא מרונדרים כהערות) | ה‑separator המותאם כמעט תמיד קו פשוט; שחזורו דורש שמירת ה‑type בקריאה. הקו ⅓‑רוחב = ברירת המחדל של Word | J | נמוכה |
| 39 | **פיצול הערה ארוכה בין עמודים לא נתמך (שלב 1)** — הערה ענקית שאינה נכנסת בעמוד שלם **נחתכת** (clamp) במקום להתפצל עם `continuationSeparator` לעמוד הבא. הערה רגילה שדוחפת שורה לעמוד הבא **כן** מטופלת | פיצול תוכן‑הערה דורש שכתוב נתיב ה‑split להזין את הרצועה השמורה; הערות קצרות (הנפוץ) נכנסות תמיד. שלב 2 עתידי | J | נמוכה |
| 40 | **הערות סיום: docEnd בלבד + מספור רציף.** ההערות זורמות בסוף המסמך (ברירת `w:pos`); המיקום `sectEnd` (סוף כל מקטע) ו‑restart `eachSect` לא מיושמים. פורמט המספור (decimal/roman/letter/hebrew) כן מכובד מ‑`w:endnotePr`. אין separator/כותרת "Endnotes" בנתיב ה‑paged (זרימה ישירה); ה‑Dialog נשאר ב‑continuous | docEnd+רציף = ברירת המחדל המכריעה; sectEnd נדיר ודורש הזרקת בלוקים פר‑מקטע | J | נמוכה |
| 41 | **שינוי מספר‑טורים `continuous` באמצע עמוד — מיקום שגוי ברינדור (לא רק חוסר‑איזון).** ה‑Paginator קורא `_applyColumnLayout` ב‑`_beginSection` ומתייג slices של המקטע החדש ב‑`columnIndex` נכון, אך הרינדור מחליט `isMultiCol` לפי `page.section.columns` — המקטע שבו ה**עמוד נפתח**. בעמוד מעורב (מקטע חד‑טורי → דו‑טורי באותו עמוד) `page.section` נשאר החד‑טורי → כל ה‑slices מרונדרים ב‑`Column` יחיד לפי הסדר, מתעלם מ‑`columnIndex` → שבירת measure≡render | תיקון מלא דורש חלוקת העמוד לתת‑אזורים פר‑מקטע (page model מודע‑מקטע). שינוי טורים `continuous` באמצע עמוד נדיר; המקרה הנפוץ (מקטע דו‑טורי שמתחיל בעמוד חדש) נכון | I | נמוכה |
| 42 | **side floats בתוך מקטע רב‑טורי אינם מרונדרים.** `_buildMultiColumnBody` מסיר רק layer floats (topAndBottom/front, שמרונדרים כ‑`Positioned` ברמת‑עמוד) ובונה `Row` של טורים; float מסוג square/tight שנרשם ל‑`page.floats` לא מצויר בנתיב הרב‑טורי, ובמדידה `_recordFloats` משתמש ב‑`_used` שמתאפס בכל טור → `anchorTop` שגוי | שילוב float עוטף בתוך פריסת‑טורים נדיר; עטיפת‑טקסט פר‑טור עם float דורשת ליבת `FloatWrapText` פר‑טור (היקף עתידי) | I | נמוכה |
| 43 | **רוחבי‑טורים מפורשים (`w:equalWidth="0"`) נלקחים verbatim — סכום שחורג גולש.** `resolveColumnWidths` מכבד את `w:w` כפי שהוא, ו‑`resolveColumnGaps` את `w:space` פר‑טור; אם `Σרוחבים + Σפערים > contentWidth` (מסמך פגום/מעוגל) ה‑`Row` חורג ונחתך (clip), ללא scale‑down. במצב equal‑width החישוב סוגר בדיוק ל‑contentWidth ואין חריגה | Word עצמו לא מקנה‑מחדש רוחבים מפורשים; verbatim שומר נאמנות. חריגה מתרחשת רק בקלט פגום | I | נמוכה |
| 44 | **STYLEREF ב‑footer מציג ראשון‑בעמוד כברירת מחדל (כמו header), לא אחרון.** Word מבחין לפי כיוון החיפוש: header (ללא `\l`) → ההתאמה ה**ראשונה** בעמוד, footer (ללא `\l`) → ה**אחרונה** (חיפוש מתחתית‑העמוד כלפי מעלה). `FieldSubstitution._styleRef` אינו יודע אם השדה ב‑header או ב‑footer ומציג תמיד ראשון (`\l` תמיד אחרון). השימוש הקלאסי — כותרת רצה של מילון/תנ"ך — הוא **header** עם זוג ברירת‑מחדל/`\l` (ראשון/אחרון), וזה כעת נכון | STYLEREF ב‑footer נדיר; ההבחנה דורשת השחלת דגל header/footer ל‑`apply`, וסמנטיקת `\l`‑ב‑footer מעורפלת בתיעוד Word. ה‑header (הנפוץ) מדויק | K | נמוכה |
| 45 | **ספרות ASCII (0‑9) מסווגות תמיד כ‑Latin בפיצול‑הכתב** (פונט `w:ascii`), גם בתוך ריצה עברית. תואם את סיווג ה‑rFonts של Word (טווח ASCII) ואת המקובל (מספרים מרונדרים בפונט לטיני). `w:hint="cs"` דורס רק תווים נייטרליים (רווחים/פיסוק), לא ספרות | סיווג ספרות כנייטרלי היה מצרף אותן לפונט העברי לפי הקשר — לרוב לא‑רצוי וחורג מ‑Word | L | נמוכה |
| 46 | **`w:bCs`/`w:iCs` חסרים נופלים ל‑`w:b`/`w:i`** (ולא ל"רגיל") עבור הקטע המורכב. הסתעפות מקריאת ISO מילולית (שבה b ו‑bCs עצמאיים), אך תואמת את המקרה הנפוץ: מסמך שמגדיר רק `w:b` ומצפה ששני הכתבים יודגשו. `bCs`/`iCs` מפורשים מכובדים תמיד | אין golden מעורב‑כתב לאימות הקצה; הנפוץ (b בלבד) מדויק | L | נמוכה |
| 47 | **תחליפים מטריים הם מיפוי‑שם בלבד** (Calibri→Carlito, Arial→Arimo, Times→Tinos, David→David Libre…) — החבילה **לא** מצרפת את ה‑clones. הם מרנדרים גליפים רק אם ה‑host רשם אותם; אחרת `fontFamilyFallback` פר‑כתב תופס. כך גם משפחות לא‑מוכרות (לא בטבלה ולא זמינות) נשמרות בשם המקורי ונשענות על ה‑fallback | אריזת פונטים = נפח חבילה; ה‑host כבר כולל את פונטיו (§L.4). הטבלה ניתנת‑להרחבה דרך `DocxViewConfig.fontSubstitutions` | L | נמוכה |
| 48 | **חיפוש paged: הדגשה בכותרת/תחתית בנתיב העצל אינה מוזרקת.** ה‑seeding של מונה‑ההדגשה ב‑`buildPageWidget` מתחיל בבלוק‑הגוף הראשון של העמוד, כך שטקסט בכותרת/תחתית (שמשוכפל פר‑עמוד, נמדד פעם אחת באינדקס) **לא מודגש** חזותית בעת חיפוש. הניווט עדיין נכון (`pageForBlock`: התאמת‑כותרת→עמוד 0, התאמת‑תחתית→עמוד אחרון). בנתיב ה‑eager הישן (`_renderPages`, עדיין בשימוש ל‑`generateWidgets`/note‑dialog) ההדגשה בכותרת עמוד 1 כן הוזרקה | מונה משותף לכותרת+גוף בנתיב העצל היה דורש בניית הכותרת עם counter פר‑עמוד; הדגשת כותרת/תחתית נדירה והניווט מטפל במקרה | M | נמוכה |
| 49 | **zoom + וירטואליזציה: `panEnabled:false` בנתיב fit‑to‑width.** כשמווירטלים תחת `enableZoom`, ה‑`InteractiveViewer` משתמש ב‑`constrained:true` + `panEnabled:false` (גרירה אנכית→גלילת ListView, pinch→zoom). כשמוגדל אפשר לגלול אנכית אך לא להזיז אופקית — מקובל כי `fitPageToWidth` כבר מכניס את העמוד לרוחב. **הקומבינציה `enableZoom` ללא `fitPageToWidth`** (עמודים בגודל מקורי שצריך לפַנֵּם אליהם) **נחרגת**: שם `canVirtualize=false` ו‑`panEnabled=true` — נשמר מודל ה‑canvas‑pan הישן (פריסה מלאה, pan חופשי), כך שאין קצוות בלתי‑נגישים. **תחושת‑המחווה בנתיב ה‑fit לא אומתה על מכשיר** (M.7) | חוסר פתרון‑מחוות נקי ל‑scroll אנכי + pan דו‑ממדי באותו widget; כשהעמוד נכנס לרוחב אין מה לפַנֵּם, וכשלא — שומרים על ה‑pan החופשי | M | נמוכה |
| 50 | **ניווט תוך‑עמודי ב‑paged גולל לראש העמוד, לא ל‑offset של ה‑match.** `nextMatch`/`previousMatch` קוראים `pageForBlock`→`_jumpToPage(page)`. כשכמה matches באותו עמוד, "הבא" לא מזיז את התצוגה — האינדיקציה היחידה היא צבע ה‑current (כתום). דאון‑גרייד מהמנגנון הישן (גלילה ל‑offset דרך alignment) | תוצאה ישירה ומכוונת של M.1 — ניווט פר‑**עמוד** (block→page) במקום מפתח‑GlobalKey פר‑בלוק; offset מדויק היה דורש להחזיר את האיתור הפר‑בלוקי שנמחק. צבע ה‑current משמש כסמן בתוך‑העמוד | M | נמוכה |
| 51 | **ניווט חיפוש ב‑continuous כשהבלוק מווירטל.** ב‑continuous, `navMatchKey.currentContext` הוא null אם הבלוק יצא מה‑viewport (בדיוק בניווט ל‑match רחוק) → `ensureVisible` לא מתבצע. ל‑paged יש estimate+snap דרך `_jumpToPage`; ל‑continuous (אין page model) אין. לא רגרסיה (התקיים גם עם ה‑per‑block keys), אך M.2 הרחיב את הווירטואליזציה ל‑continuous‑תחת‑zoom | מצב continuous משני (ברירת המחדל paged); estimate ל‑continuous דורש מיפוי בלוק→offset שאין במצב הזרימה. תועד; פתרון = גישת estimate דומה ל‑paged בעתיד | M | נמוכה |
...
| H | ציורים ועטיפה | ✅ הושלם 2026-06-18 | **סגירה (2026-06-18):** עטיפה חוצת-פסקאות (§8.2 #32) נחקרה לעומק ונדחתה מודעת — דורשת שכתוב ארכיטקטוני (ניתוק גובה float מגובה פסקה + שכבת-עמוד ל-side floats + נתיב continuous נפרד + ~5 קבצי בדיקה), בעוד היא נדירה ובחומרה נמוכה; תועדה ב-§8.2 #32. golden חסום על fixtures חסרים בצ'קאאוט (קדם-קיים, משותף ל-A–G). כל התת-חלקים המהותיים (H.1 מודל, H.2 עטיפת side פר-פסקה, H.3 תמונות/צורות/תיבות-טקסט/VML) הושלמו; שאר הסטיות (§8.2 #1,2,27,28,30,31,33–36) מודעות ובחומרה נמוכה. <br> **סשן 6 (2026-06-17): VML watermark-behind.** תמונת `w:pict` עם `position:absolute` בסגנון ה-`v:shape` ממופה כעת ל-float: `z-index<0`→`behindText` (סימן מים מאחורי הטקסט), `mso-position-horizontal/-vertical(+-relative)`→align/from, `margin-left/-top`→offset (`_parseVmlPlacement`). +4 בדיקות. <br> **סשן 5 (2026-06-17): צורות — גיאומטריית presets אמיתית + gradient.** שכתוב `shape_builder`: תוקן trig מתוצרת-בית (→dart:math), `shapePresetPath` טהור (triangle/diamond/parallelogram/trapezoid/מצולעים-רגולריים/כוכבים/חיצי-בלוק/chevron/plus/line), ellipse אמיתי, איחוד פתרון-צבע, flipH/flipV. `DocxGradientFill` (AST+reader+buildXml round-trip) + רינדור LinearGradient/RadialGradient (decoration+shader). +18 בדיקות (15 גיאומטריה/רינדור + 4 gradient reader/round-trip). סטיות §8.2 #36. <br> **סשן 4 (2026-06-16): עטיפת side אמיתית (band-reflow, §8.2 #29).** `layoutFloatWrap` (ליבה טהורה) פורס טקסט שורה-אחר-שורה לפי `lineExtent` (חיתוך span פר-offset); `FloatWrapText` מרנדר Stack של שורות+float; `Paginator._measureParagraph` מודד באותה ליבה → **measure≡render**. נתיב ה-Row הישן (`getFloatsFromParagraph`) **נמחק**; `span_factory` מדלג כל float floating. parity 7 עמ' נשמר; +8 בדיקות (core) + עדכון 6 בדיקות float ל-wrap. סטיות §8.2 #31–32. <br> **סשן 3 (2026-06-15): תיבות-טקסט + תיקוני סקירת סשן 2.** `DocxShape.textBlocks` (re-entry: הקורא מפענח `w:txbxContent` כבלוקים דרך `BlockParser`; round-trip ב‑buildXml). `ShapeBuilder` מקבל `textBlockBuilder` (callback מהגנרטור → `_generateBlockWidgets`) ומרנדר את בלוקי התיבה (clipped, top-aligned) במקום מחרוזת שטוחה; `paragraph_builder._buildInlineShape` מנותב כעת ל‑`ShapeBuilder` (איחוד נתיב, תיבה inline/side מרנדרת תוכן). **תיקוני סקירה:** measure≡render ל‑side floats (נמדדים בזרימה; רק fullWidth/layer מדולגים); ניתוב inline/Row images דרך `ImageBuilder` (crop/RAM); Flutter `>=3.27`. +7 בדיקות → **docx_creator 396 + viewer 246**. סטיות §8.2 #31. <br> **סשן 2 (2026-06-15): חיווט floats מקצה-לקצה** — `PageModel.floats` (`PlacedFloat`); ה‑`Paginator` רושם floats פר‑עמוד (rect מ‑`resolveFloatRect`, anchorTop=`_used`), topAndBottom שומר גובה (`exBottom→cursor`), ו‑floats **לא נמדדים inline** (`span_factory` מדלג floating → תוקן measure≡render); ה‑renderer מצייר שכבת `Positioned` z-order ל‑topAndBottom+front, מפשיט אותם מהגוף (`_stripFloats`, ללא רינדור כפול), behindText=רקע, side=Row-legacy. `FittedBox` מיישב יחידות לשכבה (§8.2#27). +8 בדיקות (4 paginator/measure, 4 render) → **viewer 242 ירוקות**. סטיות §8.2 #29–30. <br> **סשן 1:** H.1 מודל + H.3 תמונות + H.2 ליבה טהורה. AST: `DocxInlineImage` קיבל rotation/flipH/flipV/crop(`a:srcRect`); `DocxShape` קיבל flipH/flipV — round-trip מלא ב‑buildXml. הקורא ([inline_parser.dart](../packages/docx_creator/lib/src/reader/docx_reader/parsers/inline_parser.dart)) מפענח `a:xfrm` rot/flip + `a:srcRect` לתמונות+צורות, **ועוגן floating של צורות** (קודם הוזנח). רינדור תמונות ([image_builder.dart](../packages/docx_file_viewer/lib/src/widget_generator/image_builder.dart)): crop→flip→rotate + `cacheWidth/Height` (RAM §2.4#2). ליבה טהורה חדשה [float_layout.dart](../packages/docx_file_viewer/lib/src/layout/float_layout.dart): `resolveFloatRect` (כל relativeFrom×align/offset → קואורדינטות-גוף) + `lineExtent`/`nextUsableY` (פותר רצועות, side/topAndBottom/layer, RTL). **docx_creator 393 + viewer 234 ירוקות** (+5 +26 חדשות; 4 golden חסרות-fixture קדם-קיים). **נשאר:** חיווט float ל‑Paginator (רצועות-מדידה) + PageWidget (Positioned z-order), מחיקת Row-grouping הישן, תיבות-טקסט (re-entry לבלוקים), VML watermark, presets נוספים+gradient, יישוב יחידות §8.2#27. סטיות §8.2 #27–28. יומן 2026‑06‑15 |
| I | טורים | ✅ הושלם 2026-06-18 | **טורים מרובים** (`w:cols`): `columnIndex` ב‑`BlockSlice`, `column_layout.dart` (`resolveColumnWidths`/`columnGapPx`/`resolveColumnGaps`), `_advanceColumn`+`_activeContentWidth` ב‑Paginator, `_buildMultiColumnBody` (Row+separator+RTL) ב‑widget generator, `DocxLineBreak.isColumnBreak`+parser. **21 בדיקות** (14 עימוד + 7 רינדור). **תיקוני QA:** separator‑בלתי‑נראה (גובה 0)→גובה‑טור מפורש; `_placeGroup` `_newPage`→`_advanceColumn` (דילג טורים); `_colCount=_colWidths.length`; פער פר‑טור explicit. סטיות §8.2 #41–43. 4 golden קדם‑קיים לא מושפעים; parity 7 נשמר. |
| J | הערות שוליים | ✅ הושלם 2026-06-18 | **הערות שוליים בתחתית העמוד** (Plan §J.1–4): ה‑`Paginator` שומר רצועה בתחתית הגוף שגדלה כלפי מעלה כשהערה נכנסת לעמוד (`_footnotesBand`, `_remaining` מחסיר אותה) — הערה שלא נכנסת **דוחפת את השורה לעמוד הבא**; ה‑ref בטקסט וההערה למטה מציגים אותו מספר מחושב‑בעימוד (decimal/roman/letter/**גימטריה** דרך `NumberFormatter`, restart continuous/eachSect/eachPage); separator ⅓‑רוחב; ה‑ref וההערה תמיד באותו עמוד. **הערות סיום** (§J.5) זורמות ב‑docEnd עם מספור רציף. `PlacedFootnote`+`PageModel.footnotes/footnotesHeight`+`PaginationResult.footnote/endnoteLabels`; הרינדור מצמצם את אזור‑הגוף ברצועה ומצייר את ההערות בה (measure≡render). **6 בדיקות עימוד + 2 רינדור ירוקות**; word_parity 7=7 נשמר; 4 golden עברית חסרי‑fixture (קדם‑קיים). סטיות §8.2 #37–40. ה‑Dialog נשאר ל‑continuous. יומן 2026‑06‑18 |
| K | שדות/TOC | ✅ הושלם 2026-06-18 | **STYLEREF** תלוי‑עימוד (ראשון/`\l` אחרון בעמוד + carry‑over; התאמה לפי שם‑סגנון מ‑styles.xml — גם עברי — או styleId מנורמל), נפתר ב‑`FieldSubstitution` מ‑`PageModel.styleRefLast/First`. **TOC חי** — `expandTocBlocks` מרחיב לפסקאות‑מטמון (leader+PAGEREF חי דרך FieldSubstitution על הגוף). **קישורים** — פנימי `#anchor`→`DocxViewController.jumpToBookmark`/`bookmarkPageIndex`, חיצוני `config.onOpenLink`; `HYPERLINK` כשדה. **סמלים** `w:sym`→Unicode בינלאומי (`symbol_map.dart`: checkbox/smiley/חצים + יוונית/מתמטיקה של Symbol; לא‑ממופה→פונט מקורי+fallback). **SDT** שקוף (dropdown/checkbox). **OMML**=placeholder לינארי. docx_creator 423 + viewer 333 ירוקות (4 golden חסרי‑fixture קדם‑קיים). סטיות §K ביומן. יומן 2026‑06‑18 |
| L | פונטים | ✅ הושלם 2026-06-19 | **`FontResolver`** (`font_loader/font_resolver.dart`): שם‑Word→משפחה זמינה (override‑host→מוטמע/מערכת זמין→טבלת תחליפים מטריים→שם כפי שהוא); **fontFamilyFallback פר‑כתב** (עברית/ערבית לפני ה‑fallbacks של ה‑host). **פיצול כתב** (`classifyScript`+`resolveRunSegments`): ריצה מעורבת נחתכת לפי כתב, קטע מורכב=cs/szCs/bCs/iCs, קטע לטיני=ascii/sz/b/i; נייטרלים יורשים הקשר, hint=cs→מורכב. מודד+רינדר צורכים את אותם segments (measure≡render; `SpanSegment` נשאר אחד פר‑ריצה; הדגשת‑חיפוש/קישור פר‑קטע). **טעינה עצלה** של מוטמעים (רק אם מאוזכר; superset כולל פונטי theme). `DocxViewConfig.fontSubstitutions`. **+28 בדיקות** (font_resolver+script_split+collect_fonts); analyze נקי, parity 7 נשמר; 4 golden עברית חסרי‑fixture (קדם‑קיים). סטיות §8.2 #45–47. יומן 2026‑06‑19 |
| M | ביצועים/RAM | 🟨 קוד הושלם — נשאר אימות תקציבים על מכשיר/profile | **M.1 חיפוש ללא רגנרציה גלובלית + מחיקת GlobalKeys פר‑בלוק:** מנגנון ה‑`keyRegistry`/`registerKey` נמחק (`BlockIndexCounter` נשאר רק מונה; הבנאים לא מקצים מפתח פר‑בלוק). חיפוש paged = `setState` בלבד שמרענן את העמודים **הנראים** — `buildPageWidget` מזריע counter בבלוק‑הגוף הראשון של העמוד (`_bodyStartBlock`) ומזריק הדגשות בבנייה העצלה; ניווט = `pageForBlock` (מפת block→page) → גלילה לעמוד (עובד גם כשהבלוק מווירטל). מצב continuous = מפתח‑בודד לטץ' הפעיל. **M.2 zoom+וירטואליזציה:** `canVirtualize` כבר לא תלוי ב‑`enableZoom`; `InteractiveViewer(constrained,panEnabled:false)` שומר את ה‑`ListView.builder` (§8.2 #49 — תחושת מחווה לאימות מכשיר). **M.3 isolate:** `DocxReader.loadFromBytes` עטוף `compute` (AST sendable — נבדק); רק הפענוח עובר ל‑isolate, מדידה נשארת ב‑UI. **M.4 מטמונים:** `imageCacheMaxBytes` (50MB ברירת‑מחדל, הורדה‑בלבד) מוחל ב‑init ומשוחזר ב‑dispose; LRU מדידות מאומת 4000 (§4.3). **M.6 Timeline:** `docx.parse`/`docx.paginate.page`/`docx.page.build`. **+12 בדיקות** (search_page_map 4 — כולל אימות **צבע** הדגשה; isolate_parse 2 — round‑trip always‑run על כל סוגי ה‑node; image_cache_budget 3 — כולל race רב‑viewer; **perf_benchmark 3** — §N.4 הוקדם: `layoutCount` דטרמיניסטי נגד thrash, ו‑**חיפוש**: 40 התאמות/2000 בלוקים ב‑2ms + אי‑עימוד‑מחדש, שמכסה את החצי החישובי של תקציב ה‑≤100ms §M.7) → **viewer 373 ירוקות** (4 golden עברית חסרי‑fixture קדם‑קיים), parity 7 נשמר. סטיות §8.2 #48–51. **נשאר (דורש מכשיר/profile, לא בר‑הרצה ב‑harness):** M.5 פרופיילינג מתועד + M.7 מדידת תקציבי §2.2/§2.3 (wall‑clock/RAM) על מסמך‑ייחוס + אימות תחושת ה‑zoom. יומן 2026‑06‑19 |
...
**מענה לסקירת QA (סשן 6b):** הורחב `_parseVmlPlacement` — **rotation** מתוך ה‑`style` (watermark אלכסוני, נפוץ!), **`<w10:wrap type>`** קובע wrap ל‑float קדמי (square/tight/through/topAndBottom) במקום `none` קשיח, ו‑**`alt`** → altText. +3 בדיקות (rotation; w10:wrap=square לקדמי; behind נשאר behindText). **סטייה מודעת:** כל תמונת VML עם `position:absolute` הופכת כעת ל‑floating (קודם inline) — סמנטית נכון, אך תמונות VML‑legacy קדמיות (חתימות/OLE) שהוצגו בזרימה עלולות לעבור לשכבה; טרנספורמי `lumMod`/`lumOff` של schemeClr ו‑VML flip עדיין לא נתמכים.
**נשאר (המשך H):** עטיפה חוצת‑פסקאות (exclusion ברמת‑עמוד); golden (חסום על fixtures).

### 2026-06-18 — חלק H (ציורים ועטיפה) — ✅ הושלם (סגירה: עטיפה חוצת‑פסקאות נחקרה ונדחתה מודעת)
**הקשר:** הפריט האחרון שנותר ב‑H היה עטיפה חוצת‑פסקאות (§8.2 #32) — float גבוה שמאלץ גם את הפסקאות הבאות לעטוף סביבו. נחקר לעומק לקראת מימוש ברמה בינלאומית, ובוצע PoC לתשתית ההפצה.
- **ממצא ארכיטקטוני:** המודל הקיים (§8.2 #29) מנפח את פסקת‑העוגן לגובה ה‑float המלא (`layoutFloatWrap`→`_floatsHeight=max(תחתית‑טקסט, תחתית‑float)`), ולכן float **לעולם אינו גולש** מתחת לעוגן — הפסקה הבאה מתחילה תמיד מתחת ל‑float. כל מנגנון "הפצה לפסקאות הבאות" נשאר אינרטי כל עוד העוגן בולע את הגובה.
- **מה שנדרש (ולא בוצע):** עטיפה חוצת‑פסקאות אמיתית = שכתוב ארכיטקטוני: (1) ניתוק גובה ה‑float מגובה הפסקה (עוגן=גובה טקסט בלבד); (2) העברת **ציור** ה‑side floats מהפסקה לשכבת‑העמוד (Positioned) — אחרת ה‑float נחתך מקופסת‑העוגן הקצרה; (3) שמירת מקום ברמת‑עמוד לתחתית ה‑float; (4) שמירה על מצב continuous (אין page‑model) → שני נתיבי‑רינדור; (5) עדכון ~5 קבצי בדיקה שמקבעים את §8.2 #29 (float_render/float_pagination/float_alignment/rendering_fidelity/float_text_layout). blast‑radius רחב, רגיש‑parity — לא תוספה מוכלת.
- **PoC שהוסר:** נבדקה תשתית ההפצה — `FloatRect.translate`, accumulator פר‑עמוד ב‑paginator (`_pageSideFloats`) + ב‑renderer (`_generatePagedBody`, שחזור rects זהה ל‑paginator לשמירת measure≡render) + מסנן חפיפה אנכי + `extraExclusions` ל‑`FloatWrapText`. התשתית נכונה אך אינרטית ללא ניתוק‑הגובה; הוסרה כדי לא להשאיר קוד מת.
- **החלטה:** §8.2 #32 נשאר **סטייה מודעת בחומרה נמוכה** (נדיר ויקר; המקרה הנפוץ של תמונה+טקסט צמוד בפסקה מטופל ע"י §8.2 #29) — עקבי עם סגירת F/G. **חלק H סומן ✅** ב‑§9.
**בדיקות:** עץ נקי הוחזר (אין שינוי קוד); כל בדיקות ה‑float/paginator/parity ירוקות, **formatting‑demo=7 עמודים** נשמר. (עבודת VML של סשן 6b — rotation/w10:wrap/alt — קומיט 7e7f1a1, docx_creator 415 ירוקות.)
**שאר הסטיות המודעות של H:** §8.2 #1 (tight/through≈square), #2 (EMF/WMF placeholder), #27 (יחידות נקודות‑כ‑px בנתיב inline), #28 (float מרכזי קירוב), #30 (topAndBottom מ‑anchorTop), #31/#33 (float מרכזי כבלוק), #34 (layer floats ב‑continuous), #35 (justify∩float), #36 (preset‑geometry מורכב). golden חסום על fixtures חסרים (קדם‑קיים, משותף A–G).

### 2026-06-18 — חלק J (הערות שוליים והערות סיום על העמוד) — ✅ הושלם
**הקשר:** עד כה הערות שוליים הוצגו רק כ‑Dialog בלחיצה (ללא מיקום אמיתי על העמוד) וה‑ref בטקסט הראה את ה‑`w:id` הגולמי. מומש המודל של Word: אזור‑הערות בתחתית העמוד שגדל כלפי מעלה ומקטין את הגוף, מספור תלוי‑עימוד, וזרימת הערות‑סיום בסוף המסמך.
- **רזרבציית רצועה ב‑Paginator** ([paginator.dart](../packages/docx_file_viewer/lib/src/pagination/paginator.dart)): מצב פר‑עמוד `_pageFootnotes`/`_pageFootnoteIds`/`_footnotesBand`, ו‑`_remaining = bodyHeight − used − band`. בבדיקת ההתאמה של בלוק (`_placeBlock`) מחושב `_footnoteGrowth(newIds)` (separator על ההערה הראשונה + גובה‑תוכן כל הערה + פער) ומתווסף לגובה‑הבלוק; שורה שהערתה אינה נכנסת **נדחית לעמוד הבא**. בפיצול פסקה — מילון‑הערות הפסקה כולה נשמר מראש (שמרני, לעולם לא חורג), וה‑head מקובע רק לתת‑הקבוצה שלו. `_footnoteContentHeight` ממומו (מתבטל בשינוי רוחב‑תוכן בין מקטעים).
- **מספור תלוי‑עימוד** (§J.4): `_commitFootnote` מקצה תווית מ‑`NumberFormatter.formatPage(_footnoteNumber++, format)` בעת המסירה (סדר‑מסמך), כך ש‑restart `eachSect` (אִתחול ב‑`_beginSection`) ו‑`eachPage` (אִתחול ב‑`_openPage`) נכונים; הפורמט מ‑`w:footnotePr` של המקטע (דורס את ברירת‑המסמך). תמיכה ב‑decimal/roman/letter/**hebrew1 (גימטריה)**/hebrew2.
- **מודל נתונים** ([page_model.dart](../packages/docx_file_viewer/lib/src/pagination/page_model.dart)): `PlacedFootnote{id,label,content,height}`, `PageModel.footnotes`+`footnotesHeight`, `PaginationResult.footnoteLabels`/`endnoteLabels`.
- **רינדור** ([docx_widget_generator.dart](../packages/docx_file_viewer/lib/src/widget_generator/docx_widget_generator.dart)): אזור‑הגוף מצומצם ברצועה (`bottom: bodyBottom + footnotesHeight`) ואזור‑ההערות מצויר בה (`_buildFootnoteArea`: separator ⅓‑רוחב בקצה המוביל + כל הערה עם מספרה כ‑superscript מוביל; RTL→separator מימין). ה‑`ParagraphBuilder` מקבל `footnoteLabels`/`endnoteLabels` (מוגדרים פר‑עמוד מ‑`page.footnotes` לפני בניית הגוף — ה‑ref וההערה תמיד באותו עמוד), ו‑`_buildFootnoteRef`/`_buildEndnoteRef` מציגים את התווית.
- **הערות סיום** (§J.5): `_buildEndnoteBlocks` בונה פסקאות זורמות בסוף המסמך (סדר‑הפניה, מספר מוביל), מוזרקות אחרי לולאת המקטעים ב‑`paginate`/`paginateAsync`; `endnoteLabels` מאפשר ל‑ref בטקסט להציג את אותו מספר.
**בדיקות:** [footnote_pagination_test.dart](../packages/docx_file_viewer/test/footnote_pagination_test.dart) (6: מיקום באותו עמוד, אי‑חריגת גוף+רצועה מ‑bodyHeight, הערה דוחפת שורה לעמוד הבא, eachPage→1,1 מול continuous→1,2, גימטריה א/ב, זרימת הערות‑סיום) + [footnote_render_test.dart](../packages/docx_file_viewer/test/footnote_render_test.dart) (2: תוכן ההערה מרונדר בתחתית, גימטריה מופיעה ב‑ref ובהערה). **viewer 296 ירוקות** (288 קודם + 8 חדשות; 4 golden עברית חסרי‑fixture קדם‑קיים), `word_parity` **formatting‑demo=7** נשמר. analyze נקי, `dart format` הורץ.
**סטיות מודעות (§8.2 #37–40):** #37 גובה הרצועה נמדד ללא מספר‑ההערה המוביל (פער מדידה≡רינדור זניח; הרצועה ב‑clip‑area); #38 separator ברירת‑מחדל בלבד (ה‑reader משמיט `w:type`); #39 פיצול הערה ארוכה בין עמודים נדחה לשלב 2 (clamp); #40 הערות‑סיום docEnd+רציף בלבד (sectEnd/eachSect לא מיושמים). ה‑Dialog נשמר ל‑continuous (§J.6).
**נשאר (לא חוסם):** golden‑image (חסום על fixtures משותפים A–G); פיצול הערה (#39) ו‑separator מותאם (#38) — שלב 2 עתידי.

**מענה לסקירת QA (אותו יום):** (1) **באג נכונות** — `_endnoteNumber` לא אופס ב‑`_reset()`, כך שקריאת `paginate()` חוזרת על אותה אינסטנס (רענון/שינוי גודל) המשיכה את מספור הערות‑הסיום. תוקן (+`_fnCacheWidth = double.nan` לניקוי מלא). (2) **באג RTL** — `_buildFootnoteEntry` השתמש ב‑`CrossAxisAlignment.start` קשיח, כך שתוכן הערה בעברית יושר לשמאל; הועבר פרמטר `rtl` (יישור end ל‑RTL). (3) צבע ה‑separator נגזר כעת מצבע הטקסט (×0.45 alpha) במקום grey קשיח — נראה גם ב‑theme חשוך. (4) +3 בדיקות: eachSect restart (מקטעים), אי‑המשכיות מספור הערות‑סיום בין שתי קריאות, ורינדור הערות‑סיום. **viewer 293 ירוקות.** סטייה שנותרה מודעת: אותו `footnoteId` מופנה מכמה עמודים תחת eachPage — ההערה משוכפלת פר‑עמוד (עקבי, נדיר; §8.2). הקירוב השמרני בפיצול (head שומר את רצועת ההערות של כל הפסקה) נשאר — בטוח (לעולם לא חורג), עלול לבזבז ~separator+gap.

### 2026-06-18 — חלק I (טורים מרובים `w:cols`) — ✅ הושלם
**הקשר:** עד כה ה‑`w:cols` נקרא ל‑AST (חלק A) אך הפריסה התעלמה ממנו — כל מסמך רונדר בטור יחיד. מומשה פריסת‑טורים מלאה: עימוד פר‑טור, שבירת‑טור, וסדר טורים RTL.
- **מודל** ([block_slice.dart](../packages/docx_file_viewer/lib/src/pagination/block_slice.dart)): `BlockSlice.columnIndex` (0‑based, ברירת‑מחדל 0) — איזה טור בעמוד נושא את הפרוסה; `copyWith`/`toString` עודכנו.
- **עזר משותף** ([column_layout.dart](../packages/docx_file_viewer/lib/src/layout/column_layout.dart)): `resolveColumnWidths(cols, contentWidth)` מתרגם `DocxColumns`→`List<double>` רוחבי‑פיקסל (equalWidth: `(contentW − gaps)/n`; explicit: `widthTwips/15`), `columnGapPx`. שימוש משותף מבטיח **מדידה≡רינדור**.
- **עימוד** ([paginator.dart](../packages/docx_file_viewer/lib/src/pagination/paginator.dart)): מצב `_colCount`/`_colIndex`/`_colWidths`; `_activeContentWidth` (רוחב הטור הפעיל, נופל ל‑`contentWidth` בטור יחיד) משמש כל מדידה/פיצול (`_placeGroup`/`_placeBlock`/`_splitParagraph`/`_splitTable`); `_advanceColumn()` עובר לטור הבא או לעמוד חדש כשהאחרון מלא; `_applyColumnLayout(section)` נקרא ב‑`_openPage` וב‑`continuous` mid‑page. שבירת‑טור (`DocxLineBreak.isColumnBreak`) מפצלת את הפסקה לחלק‑שלפני בטור הנוכחי וחלק‑אחרי בטור הבא.
- **רינדור** ([docx_widget_generator.dart](../packages/docx_file_viewer/lib/src/widget_generator/docx_widget_generator.dart)): `_buildMultiColumnBody` מקבץ פרוסות לפי `columnIndex`, בונה `Row` של `SizedBox`‑ים ברוחבי הטורים עם פער/separator (קו ⅓‑אפור) ביניהם; **RTL** → היפוך סדר הטורים (טור 0 מימין).
- **AST/reader** ([docx_inline.dart](../packages/docx_creator/lib/src/ast/docx_inline.dart), [inline_parser.dart](../packages/docx_creator/lib/src/reader/docx_reader/parsers/inline_parser.dart)): `DocxLineBreak.isColumnBreak` (round‑trip `w:br w:type="column"`); ה‑parser מבדיל `column` מ‑`page`.
**בדיקות:** [column_pagination_test.dart](../packages/docx_file_viewer/test/column_pagination_test.dart) — 14 בדיקות: `resolveColumnWidths` (equal 2/3, explicit, count=1), עימוד דו‑טורי (חלוקה, גלישה לעמוד חדש, שבירת‑טור, שבירה בטור אחרון→עמוד 2, טור‑יחיד=index 0), RTL (אינדקסים זהים ל‑LTR — הרינדור הופך), `BlockSlice` (ברירת‑מחדל/copyWith/toString). כל ה‑14 ירוקות; 4 golden עברית חסרי‑fixture קדם‑קיים (משותף A–J).
**סטיות מודעות:** איזון‑טורים ל‑`continuous` (Word ממרכז את הטור האחרון) — best‑effort דרך הזרימה הרגילה, ללא איזון אמיתי.

**מענה לסקירת QA (אותו יום):** (1) **באג רינדור ודאי — קו ה‑`w:sep` היה בלתי‑נראה.** ה‑separator היה `Container(width:1)` בתוך `Center` ללא גובה — בתוך `Row` הוא מתכווץ לגובה 0. תוקן: `columnHeight = bodyHeight − footnotesHeight` (גובה אזור‑הטורים, זהה ל‑`Positioned` של הגוף) מועבר ל‑`_buildMultiColumnBody` וניתן ל‑`Container.height` → הקו משתרע לכל גובה הטור כמו ב‑Word. (זול מ‑`IntrinsicHeight` שעלול להיכשל על Stack של floats בתוכן.) (2) **`_placeGroup` דילג טורים** — קבוצת keepNext שלא נכנסה לטור עשתה `_newPage()` במקום `_advanceColumn()` (קפיצה לעמוד הבא במקום לטור הבא). תוקן. (3) **`_colCount` לא נחתך** — `_applyColumnLayout` קבע `_colCount = cols.count` בעוד `resolveColumnWidths` חותך ל‑64; שונה ל‑`_colCount = _colWidths.length` כדי שלא יחרגו זה מזה. (4) **פער פר‑טור (explicit)** — נוסף `resolveColumnGaps`: equalWidth→`w:space` כללי לכל פער; explicit→`w:col/@w:space` פר‑טור (נפילה לברירת‑מחדל) — מתקן מיקום שגוי ומקטין סיכון overflow. (5) **+7 בדיקות רינדור** ([column_render_test.dart](../packages/docx_file_viewer/test/column_render_test.dart)): `resolveColumnGaps` (equal/explicit/single), טור‑0‑משמאל‑לטור‑1 (LTR), טור‑0‑מימין (RTL — בדיקת מיקום `dx` בפועל), separator נראה בגובה>0 (חושף את באג #1), אין separator כש‑`w:sep` כבוי. **viewer 314 ירוקות** (4 golden עברית חסרי‑fixture קדם‑קיים), `word_parity` formatting‑demo=7 נשמר. **סטיות חדשות שתועדו (§8.2 #41–43):** שינוי טורים `continuous` mid‑page → מיקום שגוי ברינדור (לא רק חוסר‑איזון; הרינדור קורא `page.section` של פתיחת‑העמוד); side floats בתוך מקטע רב‑טורי לא מרונדרים; רוחבים מפורשים verbatim → חריגה בקלט פגום. הערת הקומיט הקודם ציינה "docx_creator 415" — הבדיקות החדשות הן ב‑**docx_file_viewer**.

### 2026-06-18 — חלק K (שדות, TOC, קישורים, SDT, סמלים) — ✅ הושלם (בינלאומי)
**הקשר:** עד כה רק PAGE/NUMPAGES/SECTIONPAGES/PAGEREF טופלו; STYLEREF/קישורים‑פנימיים/TOC‑חי/סמלים/OMML חסרו או נשרו בתצוגה. מומש כל חלק K עם דגש על רב‑לשוניות (עברית/ערבית/לטינית/יוונית).
- **STYLEREF (§K.3)** — תלוי‑עימוד כמו PAGE: ב‑[paginator.dart](../packages/docx_file_viewer/lib/src/pagination/paginator.dart) `_prepareStyleRefs` סורק גוף+כל וריאנטי כותרת/תחתית ל‑`DocxStyleRef`, ובונה `styleId→שם‑סגנון מנורמל` מ‑`styles.xml`. בזמן מילוי עמוד `_recordStyleRef` עוקב אחר טקסט הפסקה האחרונה/ראשונה לכל סגנון נעקב (carry‑over בין עמודים); ב‑`_closePage` נשמרים `PageModel.styleRefLast/styleRefFirst`. ברירת‑מחדל=ראשון‑בעמוד, `\l`=אחרון‑עד‑סוף‑העמוד (סמנטיקת Word: "\l … instead of the first"). התאמה לפי **שם‑סגנון** (גם עברי) או styleId מנורמל (`"Heading 1"`↔`Heading1`). [field_substitution.dart](../packages/docx_file_viewer/lib/src/pagination/field_substitution.dart) פותר `DocxStyleRef` מ‑`PageContext` (fallback ל‑cachedText).
- **קישורים (§K.2)** — `w:hyperlink w:anchor` ו‑`HYPERLINK \l` → `href="#name"`; חיצוני → url. ב‑[paragraph_builder.dart](../packages/docx_file_viewer/lib/src/widget_generator/paragraph_builder.dart) הקשה מנותבת: פנימי→`onInternalLink`→גלילה לעמוד מ‑`bookmarkPageIndex`; חיצוני→`config.onOpenLink` (נפילה ל‑url_launcher). API חדש: [DocxViewController](../packages/docx_file_viewer/lib/src/docx_view_controller.dart) `jumpToBookmark`/`jumpToPage`/`bookmarks`, מחווט ל‑`DocxView` עם `ScrollController`.
- **TOC חי (§K.1)** — [toc_expander.dart](../packages/docx_file_viewer/lib/src/pagination/toc_expander.dart) מרחיב `DocxTableOfContents`→פסקאות‑המטמון לפני עימוד+רינדור (גם רציף+חיפוש), כך שהן זורמות בנתיב הרגיל (leader tabs דרך TabbedLine). FieldSubstitution הוחל גם על גוף העמוד → PAGEREF חי במקום מטמון.
- **סמלים (§K.5)** — [symbol_map.dart](../packages/docx_file_viewer/lib/src/layout/symbol_map.dart) ממפה `w:sym` (Wingdings/Symbol בלבד) ל‑Unicode שקול (✓✗☑☺ חצים + יוונית/מתמטיקה מלאה של Symbol). תו ממופה→פונט גוף (נראה גם בלי הפונט המקורי); לא‑ממופה→הפונט המקורי + fontFamilyFallback. `DocxSymbol` עתה מרונדר ב‑`buildInlineSpans` (קודם נשר).
- **שדות/SDT/OMML (reader, §K)** — [field_instruction.dart](../packages/docx_creator/lib/src/reader/docx_reader/parsers/field_instruction.dart): STYLEREF→`DocxStyleRef`, `parseHyperlink`. [inline_parser.dart](../packages/docx_creator/lib/src/reader/docx_reader/parsers/inline_parser.dart): `_fieldNodes` הופך HYPERLINK לקישור; `w:hyperlink/@w:anchor`→`#`; OMML inline→טקסט לינארי. [block_parser.dart](../packages/docx_creator/lib/src/reader/docx_reader/parsers/block_parser.dart): OMML‑בלוק→פסקה. SDT כבר נפרס שקוף (dropdown=הטקסט הנבחר, checkbox=☐/☒). `DocxText.buildXml` round‑trip ל‑`#anchor`. AST חדש: `DocxStyleRef` ([docx_section.dart](../packages/docx_creator/lib/src/ast/docx_section.dart)).
**בדיקות:** docx_creator [field_parsing_test.dart](../packages/docx_creator/test/field_parsing_test.dart) +9 (STYLEREF/parseHyperlink/HYPERLINK‑field/anchor/OMML) → **423 ירוקות**. viewer +19: [field_substitution_test.dart](../packages/docx_file_viewer/test/field_substitution_test.dart) (STYLEREF), [symbol_map_test.dart](../packages/docx_file_viewer/test/symbol_map_test.dart), [toc_expander_test.dart](../packages/docx_file_viewer/test/toc_expander_test.dart), [styleref_pagination_test.dart](../packages/docx_file_viewer/test/styleref_pagination_test.dart) (כולל שם‑סגנון עברי + carry‑over), [link_routing_test.dart](../packages/docx_file_viewer/test/link_routing_test.dart) → **333 ירוקות** (4 golden עברית חסרי‑fixture קדם‑קיים). analyze נקי, `dart format` הורץ. `xml` הועבר ל‑dependency (היה dev‑only) לפענוח `styles.xml` ב‑paginator.
**סטיות מודעות:** (1) STYLEREF במספר עמודים תחת אותו סגנון בפיצול פסקה — ה‑head נושא טקסט חלקי (נדיר; כותרות לא מתפצלות). (2) OMML=placeholder לינארי בלבד (אין פריסת נוסחה — מודע, §K.6). (3) STYLEREF `\n/\w/\r/\t/\p` (מספור פסקה) לא ממומשים — רק טקסט. (4) ניווט מיישר את **ראש העמוד** של הסימנייה לראש התצוגה (לא את ה‑offset המדויק של הסימנייה בתוך העמוד — סימנייה באמצע עמוד תופיע מעט נמוך). (5) golden עברי של K חסום על fixtures משותפים A–G.

**מענה לסקירה (אותו יום) — תיקון ניווט קישורים:** דווח שלחיצה על קישור פנימי **איטית** וקופצת ל**תחתית** העמוד במקום לראשו. שני באגים תוקנו ב‑[docx_view.dart](../packages/docx_file_viewer/lib/src/docx_view.dart): (1) **מיקום שגוי** — חישוב‑offset מוערך (`index×slotH×scale` עם clamp ל‑`maxScrollExtent`) חרג/נחתך → נחיתה בתחתית. הוחלף ב‑`Scrollable.ensureVisible(pageContext, alignment: 0.0)` על `GlobalKey` פר‑עמוד — מיישר **מדויק** את ראש‑העמוד לראש‑התצוגה, ללא תלות ב‑zoom/fit; ההערכה נשארת רק כ‑fallback לעמוד שטרם נבנה (וירטואליזציה), עם תיקון‑frame אחרי הבנייה. (2) **איטיות** — לחיצה השתמשה באנימציה 300ms (fly‑through מנקר בעמודים, כבד ב‑SingleChildScrollView הלא‑מווירטל של מצב zoom ברירת‑מחדל). לחיצת‑קישור עתה **מיידית** (`Duration.zero`); `DocxViewController.jumpToBookmark` שומר ברירת‑מחדל 300ms חלקה לקריאות תוכנתיות. `_pageKeys` מתנקה בכל טעינה. analyze נקי; **viewer 333 ירוקות** (4 golden קדם‑קיים).

**תיקון המשך — assertion ב‑`animateTo`:** נתיב ה‑fallback (עמוד לא‑בנוי, בעיקר במצב מווירטל `enableZoom:false`) קרא `ScrollController.animateTo(..., Duration.zero)` — `animateTo` טוען `duration > Duration.zero` ולכן זרק `Failed assertion: 'duration > Duration.zero'` והלחיצה לא ניווטה. תוקן: משך‑אפס → `jumpTo` (קפיצה מיידית) במקום `animateTo`; נתיב ה‑`ensureVisible` הראשי וה‑post‑frame כבר מטפלים ב‑`Duration.zero` פנימית (jumpTo).

### 2026-06-19 — חלק L (פונטים: התאמה, fallback מטרי, פיצול כתב) — ✅ הושלם
**הקשר:** עד כה `resolveRunStyle` בחר **תמיד** את פונט ה‑`ascii` והתעלם מ‑`w:cs` (תועד כמגבלה ב‑יומן 2026‑06‑14), כך שריצה מעורבת "עברית+אנגלית" קיבלה פונט/גודל **שגוי** לקטע אחד — בדיוק הבעיה שחלק L נועד לסגור. מומשו שלושת הרכיבים: פיצול‑כתב, `FontResolver`, ו‑fallback פר‑כתב, פלוס טעינה עצלה — **בלי לשבור parity** (formatting‑demo=7 עמודים) ו‑measure≡render.
- **§L.2 — `FontResolver`** ([font_resolver.dart](../packages/docx_file_viewer/lib/src/font_loader/font_resolver.dart), חדש, טהור): שם‑Word→משפחה זמינה לפי סדר: (1) override מפורש של ה‑host (`DocxViewConfig.fontSubstitutions`, intent מנצח); (2) הפונט המבוקש **זמין** (מוטמע דרך `EmbeddedFontLoader` או מערכת דרך `FontMetricsRegistry`) → נשמר ל‑1:1; (3) טבלת תחליפים **מטריים** מובנית (Calibri→Carlito, Cambria→Caladea, Times New Roman→Tinos, Arial→Arimo, Courier New→Cousine, David→David Libre, Narkisim/FrankRuehl→Frank Ruhl Libre, Georgia→Gelasio…); (4) אחרת — השם כפי שהוא, ושרשרת ה‑fallback תופסת חוסרים. ה‑`isAvailable` ניתן‑להזרקה (בדיקות).
- **§L.1 — פיצול כתב** ([font_resolver.dart](../packages/docx_file_viewer/lib/src/font_loader/font_resolver.dart) `classifyScript` + [span_factory.dart](../packages/docx_file_viewer/lib/src/layout/span_factory.dart) `resolveRunSegments`): התוכן (אחרי טרנספורם caps) נחתך לריצות‑כתב מקסימליות — מורכב (עברית/ערבית/סורית/תאנה+טפסי‑מצג) מול לטיני (אותיות לטיניות/יווניות/קיריליות + ספרות ASCII). תווים נייטרליים (רווחים/פיסוק) **יורשים את הכתב הקודם** (פסיק אחרי מילה עברית נשאר עברי), עם `w:hint="cs"` שמכריע נייטרלים→מורכב. `resolveRunStyle` קיבל פרמטר `script`: קטע מורכב משתמש ב‑`w:cs`/`w:szCs`/`w:bCs`/`w:iCs` (כל אחד נופל ל‑מקבילו הלטיני כשחסר — מסמכים שמגדירים רק `w:b`/`w:sz`), קטע לטיני ב‑`w:ascii`/`w:sz`/`w:b`/`w:i`. ריצה חד‑כתבית → segment יחיד **זהה‑בייט** לפני‑L.
- **§L.3 — fontFamilyFallback פר‑כתב:** קטע מורכב מקבל שרשרת עברית/ערבית (David Libre, Frank Ruhl Libre, Noto Sans/Serif Hebrew, Noto Naskh Arabic, Arial) **לפני** ה‑fallbacks של ה‑host, כך שתו עברי חסר‑בפונט לא נופל ל‑tofu/פונט לטיני; קטע לטיני מקבל **רק** את ה‑fallbacks של ה‑host (כך פריסת לטינית חד‑כתבית לא משתנה). `height` (יחס‑שורה) נגזר מהפונט ה**מפוענח**.
- **חיווט measure≡render:** גם המודד (`buildMeasurementSpans`) וגם הרינדר (`_buildTextSpan`) צורכים את אותו `resolveRunSegments` — מספר‑ה‑spans וסגנונותיהם זהים. ה‑`SpanSegment` של מיפוי‑הפיצול נשאר **אחד פר‑ריצה** (הקטעים מרצפים את התוכן בדיוק → ה‑offsets לא משתנים). הדגשת‑חיפוש/קישור/`w:bdr` מורבדים פר‑קטע (`_overlaySegment`) — גאומטריה‑נייטרלי, התנהגות החיפוש זהה לקודם.
- **§L.5 — טעינה עצלה** ([docx_view.dart](../packages/docx_file_viewer/lib/src/docx_view.dart)): פונט מוטמע נטען רק אם המסמך **באמת מאזכר** אותו (`collectDocumentFontFamilies`, מורחב לכלול את פונטי ה‑major/minor של ה‑theme — superset בטוח שמכסה שימוש דרך סגנון/theme). חוסך RAM במסמכים שמטמיעים פונטים רבים ונוגעים במעטים.
- **§L.4 — אריזת פונטים:** החבילה **לא** מצרפת פונטים (גודל). תועד ב‑README של החבילה כיצד ה‑host רושם פונטים מומלצים ומשתמש ב‑`fontSubstitutions`.

**בדיקות:** `docx_file_viewer`: [font_resolver_test.dart](../packages/docx_file_viewer/test/font_resolver_test.dart) חדש (17: `classifyScript` — עברית/לטינית/3‑קטעים/ריצוף/ספרות/פיסוק/hint=cs; `resolve` — null/clones/case/unknown/available‑מנצח‑clone/override‑מנצח; `fallbacksFor`), [script_split_test.dart](../packages/docx_file_viewer/test/script_split_test.dart) חדש (10: DoD "שלום Hello עולם" ascii=Arial/cs=David/sz12/szCs14→3 קטעים עם פונט+גודל נכון, b/bCs+i/iCs פר‑קטע, שרשרת עברית, `buildMeasurementSpans` segment‑יחיד אך spans פר‑כתב, **measure≡render** לפסקה מעורבת), +1 ב‑[collect_fonts_test.dart](../packages/docx_file_viewer/test/collect_fonts_test.dart) (פונטי theme ב‑superset). **+28 בדיקות; כל הסוויטה ירוקה למעט 4 golden עברית חסרי‑fixture (קדם‑קיים A–K).** `flutter analyze` נקי, `dart format` הורץ, **formatting‑demo=7 עמודים** נשמר, בדיקת ה‑Mixed Hebrew+English הקיימת ב‑`text_measurer` עוברת כעת דרך נתיב הפיצול. `docx_creator` לא נגע.

**החלטות/סטיות (§8.2 #45–47):**
1. **#45 — ספרות ASCII (0‑9) מסווגות לטיני** (לא נייטרלי): תואם את סיווג ה‑rFonts של Word (טווח ASCII→פונט ascii) ואת המקובל (מספרים בעברית מרונדרים בפונט לטיני). hint=cs **לא** דורס ספרות (הן לא‑אמביגואיות).
2. **#46 — `w:bCs`/`w:iCs` חסרים נופלים ל‑`w:b`/`w:i`** (ולא ל"לא‑מודגש"): המקרה הנפוץ הוא מסמך שמגדיר רק `w:b` ומצפה ששני הכתבים יודגשו; כש‑`bCs` מפורש הוא מכובד. הסתעפות מ‑ISO המילולית, אך תואמת‑Word בפועל.
3. **#47 — שמות תחליפים מטריים הם שמות בלבד** (Carlito/Arimo/Tinos/David Libre…): הם מרנדרים גליפים רק אם ה‑host רשם את ה‑clone; אחרת שרשרת ה‑fallback תופסת. החבילה לא מצרפת אותם.

**נשאר (נדחה, לא חוסם):** ריווח‑שורה‑מרובה (double) פר‑פונט (קדם‑קיים, יומן 2026‑06‑14); golden מעורב‑כתב אמיתי (חסום על fixtures משותפים A–K); EastAsia כ‑כתב נפרד (מקופל ל‑latin — מחוץ ליעד ה‑bidi של החבילה).
**ל‑AI הבא:** חלק M (קשיחות ביצועים/RAM) או חלק N (אימות 1:1 + fixtures) — שניהם דורשים מסמכי‑מכשיר/profile מהמשתמש.

**מענה לסקירת QA (אותו יום):** סקירה חיצונית אישרה measure≡render, את האינווריאנט "SpanSegment אחד פר‑ריצה", סדר‑resolution ותאימות‑לאחור ללטינית. טופלו שני פריטי‑קדם‑שחרור + אופטימיזציה:
1. **🟠 פער כיסוי בטעינה העצלה (correctness):** פונט מוטמע המאוזכר **רק** בתוך תיבת‑טקסט/צורה או כפונט‑סמל (`w:sym`) דולג ע"י `_collectFromInlines` → מצב‑כשל חדש (tofu, לא רק מטריקה שגויה). תוקן: ה‑collector יורד עתה גם ל‑`DocxShape.textBlocks` (רקורסיה), ל‑`DocxSymbol.font`, ול‑`DocxUnknownField.cachedResult`. **מגבלה שנותרה (מתועדת):** פונט‑bullet של הגדרת‑מספור (rPr ב‑numbering.xml) אינו נסרק — מבנה נפרד, ופונטי‑סמלים מוטמעים נדירים; אם יתגלה מקרה אמיתי, להוסיף סריקת `doc` numbering.
2. **🟢 ביצועים — `classifyScript` early‑exit:** המקרה הנפוץ (ריצה חד‑כתבית) הקצה `List<int>.filled(n)`+3 מעברים. נוסף מעבר‑סריקה זול אחד; כשאין מעבר‑כתב (לטיני‑בלבד / עברי‑בלבד / כל‑נייטרלי, או hint‑complex ללא לטינית) מוחזרת ריצה אחת **ללא הקצאה**. הנתיב המלא רץ רק לריצות מעורבות באמת.
3. **נדחה (מתועד, marginal):** memoization ל‑`resolve()` ופירוק `resolveRunStyle` לחלק כתב‑נייטרלי משותף — ה‑early‑exit הסיר את העלות העיקרית; ריצות מעורבות (מיעוט) בונות סגנון 2–3× אך המדידה ממוקאשת פר‑פסקה. אימות שינוי‑ההתנהגות בריצה עברית‑טהורה (`szCs≠sz`/`bCs≠b`) מול מדגם מסמכים רחב — דורש מסמכי‑מכשיר (כמו §2/§N).
**בדיקות:** +1 ([collect_fonts_test.dart](../packages/docx_file_viewer/test/collect_fonts_test.dart): פונט בתיבת‑טקסט + `w:sym`). **viewer 361 ירוקות** (4 golden עברית חסרי‑fixture קדם‑קיים), analyze נקי, format הורץ על הקבצים שנגעו, **formatting‑demo=7** נשמר.

### 2026-06-19 — חלק M (קשיחות ביצועים ו‑RAM) — 🟨 קוד הושלם
**הקשר:** חלק M אוכף את תקציבי §2 ומתקן את החריגות הידועות. חמישה מתוך שבעת הסעיפים בני‑מימוש ב‑harness (M.1–M.4, M.6); שניים (M.5 פרופיילינג, M.7 מדידת תקציבים/אימות Word) דורשים מכשיר/profile מהמשתמש ואינם בר‑הרצה כאן — כמו D/E. הקוד הושלם ונבדק; נשארה רק המדידה על מכשיר.

- **M.1 — חיפוש בלי רגנרציה גלובלית + מחיקת מנגנון ה‑GlobalKey פר‑בלוק (הליבה).** מחקתי את `keyRegistry`/`registerKey` מ‑[block_index_counter.dart](../packages/docx_file_viewer/lib/src/utils/block_index_counter.dart) (נשאר מונה‑בלבד עם בנאי‑זריעה `BlockIndexCounter([seed])`); הבנאים ([paragraph_builder.dart](../packages/docx_file_viewer/lib/src/widget_generator/paragraph_builder.dart), [list_builder.dart](../packages/docx_file_viewer/lib/src/widget_generator/list_builder.dart)) כבר לא מקצים `GlobalKey` פר‑בלוק (וגם פרמטרי ה‑`key` המתים הוסרו). חיפוש ב‑paged עכשיו = `setState` בלבד: `buildPageWidget` מזריע counter בבלוק‑הגוף הראשון של העמוד (`_bodyStartBlock`/`_ensureSearchPageMap`) ומזריק הדגשות בבנייה העצלה רק לעמודים **הנראים** — אין בניית כל המסמך מחדש. ניווט: `pageForBlock` (מפת block→page שנבנית פעם אחת מהעימוד; כותרת→עמוד 0, תחתית→עמוד אחרון) → `_jumpToPage` (משתמש במפתחות הפר‑**עמוד** הקיימים, עובד גם כשהבלוק מווירטל). מצב continuous: מפתח‑בודד על בלוק הטץ' הפעיל (`navMatchKey`, לא registry). ([docx_view.dart](../packages/docx_file_viewer/lib/src/docx_view.dart) `_onSearchChanged` שוכתב; [docx_widget_generator.dart](../packages/docx_file_viewer/lib/src/widget_generator/docx_widget_generator.dart)).
- **M.2 — zoom חי עם וירטואליזציה (§4.1).** `canVirtualize` כבר לא נשלל ע"י `enableZoom` (רק `pageWidth != null` הlegacy נופל ל‑SingleChildScrollView). ה‑`InteractiveViewer` משתמש ב‑`constrained:true` (נותן ל‑ListView גודל סופי) + `panEnabled:false` (גרירה אנכית→גלילת ListView, pinch→zoom). סטייה §8.2 #49 (אין pan אופקי כשמוגדל; תחושת מחווה לאימות מכשיר).
- **M.3 — isolate parsing.** `DocxReader.loadFromBytes` עטוף `compute` ([docx_view.dart](../packages/docx_file_viewer/lib/src/docx_view.dart)); רק הפענוח (unzip+XML+AST) עובר ל‑isolate, ה‑`TextPainter` נשאר ב‑UI thread. בדיקת sendability ([isolate_parse_test.dart](../packages/docx_file_viewer/test/isolate_parse_test.dart)) מאשרת ש‑`DocxBuiltDocument` עובר את גבול ה‑isolate ושווה לפענוח מקומי.
- **M.4 — מטמונים.** `DocxViewConfig.imageCacheMaxBytes` (ברירת מחדל 50MB, §4.2) מוחל ב‑`initState` (**הורדה בלבד** — לא מעלה תקרה גבוהה יותר של ה‑host) ומשוחזר ב‑`dispose`. תקרת ה‑LRU של המדידות אומתה 4000 (§4.3, [text_measurer.dart](../packages/docx_file_viewer/lib/src/layout/text_measurer.dart) `_put`); ה‑`TextMeasurer` כבר מנקה את ה‑painter+cache ב‑dispose פר‑עימוד. אין מטמון בלתי‑מוגבל אחר (spans/styles נבדקו).
- **M.6 — Timeline.** `docx.parse` (`TimelineTask` סביב ה‑compute), `docx.paginate.page` (`Timeline.instantSync` ב‑`_closePage`, [paginator.dart](../packages/docx_file_viewer/lib/src/pagination/paginator.dart)), `docx.page.build` (`Timeline.timeSync` סביב הבנייה העצלה פר‑עמוד) — מאחורי ה‑Timeline של dart:developer (no‑op ב‑release, לא debugPrint, §2.4#9).

**בדיקות:** +7 — [search_page_map_test.dart](../packages/docx_file_viewer/test/search_page_map_test.dart) (4: `pageForBlock` ראשון/אחרון פר‑עמוד מול העימוד האמיתי, היסט כותרת→עמוד 0, ‑1 לפני עימוד, והדגשת טץ' בעמוד מאוחר בבנייה עצלה), [isolate_parse_test.dart](../packages/docx_file_viewer/test/isolate_parse_test.dart) (1: compute round‑trip), [image_cache_budget_test.dart](../packages/docx_file_viewer/test/image_cache_budget_test.dart) (2: הורדה+שחזור, אי‑העלאה מעל תקרת host נמוכה). **viewer 368 ירוקות** (361 קודם + 7; 4 golden עברית חסרי‑fixture קדם‑קיים), `flutter analyze` נקי, `dart format` הורץ, **formatting‑demo=7** נשמר. `docx_creator` לא נגע.

**החלטות/סטיות (§8.2 #48–49):**
1. **#48** — בנתיב החיפוש העצל, ה‑seeding מזריק הדגשות לבלוקי‑הגוף בלבד; טקסט בכותרת/תחתית לא מודגש חזותית (הניווט נכון). מונה משותף כותרת+גוף בנתיב העצל היה דורש בניית הכותרת עם counter פר‑עמוד — נדיר ולא חוסם.
2. **#49** — `panEnabled:false` מאפשר ל‑zoom לחיות עם הווירטואליזציה אך מבטל pan אופקי כשמוגדל; `fitPageToWidth` מכסה את הנפוץ.
3. **continuous נשמר:** רשימת ה‑widgets השטוחה עדיין נבנית מחדש בחיפוש (כדי להזריק הדגשות), אך עם מפתח‑בודד לטץ' הפעיל במקום registry פר‑בלוק.

**בעיות פתוחות / נשאר ל‑✅ (דורש מכשיר/profile, כמו D/E):**
- **M.5/M.7** — להריץ על מסמך הייחוס ב‑`--profile`: זמן טעינה (≤1.5s/3s debug), עימוד 200 עמ' (≤6s ללא הקפאה), בניית‑עמוד‑בגלילה (≤8ms), 60fps, **חיפוש ≤100ms**, שיא RAM (≤200MB), מצב יציב (≤120MB). לתעד מול תוויות ה‑Timeline שנוספו. כל חריגה = משימת תיקון בתוך החלק.
- **אימות תחושת ה‑zoom** (§8.2 #49) על מכשיר מגע — שגרירה אנכית גוללת ו‑pinch מזום בלי קונפליקט.
**ל‑AI הבא:** אם יש מכשיר/מסמך‑ייחוס — להשלים M.5/M.7 (profile) ולסגור את M ל‑✅; אחרת חלק N (אימות 1:1 + fixtures), שגם דורש מסמכים מהמשתמש.

**הקשחה (אותו יום) — מענה לשאלת "רמה עולמית":** ביקורת‑עצמית העלתה שהליבה האמפירית (M.5/M.7) חסרה ושכמה בדיקות היו רכות. תוקן מה שניתן ב‑harness:
1. **`perf_benchmark_test.dart` (§N.4 הוקדם):** רף‑רך נגד רגרסיות. ה‑assert הראשי **דטרמיניסטי ולא תלוי‑מכונה** — `TextMeasurer.layoutCount` (מספר ה‑layouts האמיתיים = החטאות מטמון) חייב להישאר ~לינארי במספר הבלוקים (`< 2×`), כך שמדידה‑חוזרת/thrash או O(n²) ייתפסו; דיאגנוסטיקה (`[perf]`) מודפסת ל‑log של §M.5. מדידה בפועל: 500 בלוקים → 75 עמ', **566 layouts** (≈1.13×, מאשר מדידה‑אחת‑לבלוק), 427ms. תקרת‑הזמן (20s) היא backstop קטסטרופי בלבד — **לא** תקציב §2.2 (שעדיין דורש `--profile` על מכשיר). זיהוי נלווה: parse של 17KB = ~680ms בריצה ראשונה (כולל warmup) — מועמד לבדיקה בפרופיל.
2. **isolate_parse:** נוספה בדיקת sendability **שתמיד רצה** (בונה `DocxBuiltDocument` מייצג — פסקאות/טבלה/רשימה/הערה/כותרת — ו‑`compute(echo,doc)`), כך שאי‑sendability ייתפס גם ב‑checkout נקי בלי ה‑fixture המקומי.
3. **search_page_map:** בדיקת ההדגשה חוזקה — מאמתת שה‑span של הטץ' נושא **צבע רקע הדגשה** (`orange.shade300` לטץ' הנוכחי), לא רק שהטקסט קיים.

**אומת מחדש:** ה‑`TextMeasurer` הוא **LRU אמיתי** (re‑insert ב‑hit + בדיקת `identical(source)` נגד collision של `identityHashCode`), לא FIFO. **תיעוד‑אמת:** M **אינו ✅** — לפי ה‑DoD של עצמו (§0.4: "תקציבי §2 נמדדו ועומדים") חסרה המדידה על מכשיר; הקוד הושלם ונבדק, האכיפה האמפירית פתוחה. סטייה מודעת שנותרה: **M.2 שינה את מודל המחוות בנתיב ברירת‑המחדל (`enableZoom=true`) בלי אימות מגע** — הוחלט (המשתמש) להשאיר + לתעד (§8.2 #49). באג קדם‑קיים שזוהה (מחוץ להיקף M): צבעי ההדגשה מקודדים‑קשיח (`orange`/`yellow`) במקום `config.searchHighlightColor`. **viewer 371 ירוקות**, analyze נקי, format הורץ.

**מענה לסקירת QA (אותו יום) — קומיט dc43bff:** סקירה אישרה את עקביות counter↔index (כולל פיצול פסקה/טבלה), את השימוש החוזר ב‑`_jumpToPage`, ואת מחיקת ה‑registry. טופלו שני החוסמים שצוינו + תיקוני נכונות זולים:
1. **🔴 race ב‑imageCache (multi‑viewer) — תוקן.** המעבר מ‑save/restore פר‑instance ל‑**reference‑counting סטטי**: המופע הראשון שומר את תקרת ה‑host, כל מופע מוריד לתקציבו (התוקפני מנצח), ורק המופע **האחרון** שמתפרק משחזר. נסגר התרחיש שבו A.dispose משחזר 100MB בעוד B חי (איבוד התקרה) או דליפת 50MB אחרי שכולם נעלמו. +בדיקה (`two viewers...`: שני viewers עם keys, dispose של אחד→עדיין capped, dispose של השני→שוחזר).
2. **🔴 פרצת sendability ב‑CI — תוקן.** ה‑AST הסינתטי ב‑`isolate_parse_test` הורחב לכלול `DocxInlineImage`/`DocxShape`/`DocxText(href)`/`DocxPageNumber`/`DocxUnknownField`/`DocxSymbol`/`DocxImage` — בדיוק הסוגים שעלולים להחזיק שדה לא‑sendable — כך שרגרסיה תיתפס ב‑CI ולא רק ב‑runtime על מכשיר.
3. **§3 zoom ללא fitPageToWidth — תוקן.** `canVirtualize` נחרג כשי `enableZoom && !fitPageToWidth`: שם נשמר מודל ה‑canvas‑pan (לא‑מווירטל, `panEnabled:true`) כך שקצוות עמוד בגודל מקורי נגישים; `panEnabled:false` חל רק בנתיב ה‑fit‑to‑width המווירטל (§8.2 #49 עודכן).
4. **§3 overhead של compute — סף.** מסמך < 256KB מפוענח **inline** (spawn+copy של isolate יקר יותר מפענוח כמה KB); ≥ 256KB → `compute`. סף היוריסטי לכיול בפרופיל (§M.7).
5. **benchmark הודק** מ‑`2×` ל‑`1.5×` (566/500≈1.13×, headroom נשמר; דטרמיניסטי).
6. **תועד בלבד (follow‑up):** §8.2 **#50** ניווט תוך‑עמודי ב‑paged גולל לראש העמוד (תוצאת M.1, צבע ה‑current כסמן); **#51** ניווט continuous כשהבלוק מווירטל (אין estimate; לא רגרסיה). regeneration פר‑ניווט ב‑continuous נשאר (מצב משני). **viewer 372 ירוקות**, analyze נקי, format הורץ.

**הקשחה נוספת (אותו יום) — "עד קצה ה‑harness":** נסגר הפער שנותר בר‑מדידה ב‑harness מתוך M.7 — **בנצ'מרק חיפוש** ב‑`perf_benchmark_test`: סורק 2000 בלוקים, מאמת `matchCount>0`, את החצי החישובי של תקציב ה‑≤100ms (40 התאמות ב‑**2ms**), ושבניית עמודים תחת חיפוש פעיל **לא מעמדת מחדש** (`identical(lastPagination)` — נעילת אינווריאנט §M.1/§2.4.6). זהו ה‑proxy הדטרמיניסטי הטוב ביותר ל‑"חיפוש ≤100ms בלי רגנרציה"; מה שנותר (rebuild של עמודים נראים, wall‑clock אמיתי, RAM) הוא render על מכשיר. **viewer 373 ירוקות**, analyze נקי. **בהישג‑יד שלא נסגר מכוון (follow‑up, לא חוסם):** (א) צבעי ההדגשה הקשיחים (§ מחוץ ל‑M — מומלץ כתיקון נפרד); (ב) estimate לניווט continuous (#51); (ג) דיוק תוך‑עמודי ב‑paged (#50 — דורש להחזיר איתור פר‑בלוק שנמחק במכוון). הליבה החסומה היחידה: profile על מכשיר (M.5/M.7).
