...
6. **תועד בלבד (follow‑up):** §8.2 **#50** ניווט תוך‑עמודי ב‑paged גולל לראש העמוד (תוצאת M.1, צבע ה‑current כסמן); **#51** ניווט continuous כשהבלוק מווירטל (אין estimate; לא רגרסיה). regeneration פר‑ניווט ב‑continuous נשאר (מצב משני). **viewer 372 ירוקות**, analyze נקי (3 info קדם‑קיימים של `unnecessary_import` בקבצי בדיקה של חלק K — לא נגעתי), format הורץ.
