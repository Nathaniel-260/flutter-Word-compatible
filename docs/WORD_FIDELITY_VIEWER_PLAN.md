...
| I | טורים | ✅ הושלם 2026-06-18 | **טורים מרובים** (`w:cols`): `columnIndex` ב‑`BlockSlice`, `column_layout.dart` (`resolveColumnWidths`/`columnGapPx`), `_advanceColumn`+`_activeContentWidth` ב‑Paginator, `_buildMultiColumnBody` (Row+separator+RTL) ב‑widget generator, `DocxLineBreak.isColumnBreak`+parser, `column_pagination_test.dart` (14 בדיקות). 4 golden קדם‑קיים לא מושפעים. |
...
**סטיות מודעות:** איזון‑טורים ל‑`continuous` (Word ממרכז את הטור האחרון) — best‑effort דרך הזרימה הרגילה, ללא איזון אמיתי; `w:cols` עם רוחבים מפורשים נתמך, אך התעלמות מ‑`w:sep` פר‑טור בודד (רק `separator` כללי).
