...
| 49 | **zoom + וירטואליזציה: `panEnabled:false`.** כדי לשמר `ListView.builder` תחת `enableZoom`, ה‑`InteractiveViewer` משתמש ב‑`constrained:true` (ברירת מחדל — נותן ל‑ListView גודל סופי לווירטואליזציה) ו‑`panEnabled:false` (גרירה אנכית עוברת ל‑ListView לגלילה; pinch עדיין מזום). תוצאה: כשמוגדל אפשר לגלול אנכית אך **לא להזיז אופקית** לקצוות חתוכים. `fitPageToWidth` מכסה את המקרה הנפוץ (העמוד נכנס לרוחב). **תחושת‑המחווה עצמה לא אומתה על מכשיר** (M.7) | חוסר פתרון‑מחוות נקי ל‑scroll אנכי + pan דו‑ממדי באותו widget; הווירטואליזציה (RAM ∝ עמודים נראים) מנצחת את ה‑pan החופשי לפי §2.4/§4.1 | M | נמוכה |
