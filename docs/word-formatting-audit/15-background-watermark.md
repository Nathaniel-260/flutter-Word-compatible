...
>
> **סטטוס סריקה:** ✅ נסקר במלואו &nbsp;|&nbsp; **סטטוס מימוש:** 🔄 displayBackgroundShape (3) מומש (משימה 14); textpath/watermark נותרו נדחים &nbsp;|&nbsp; **עודכן לאחרונה:** 2026-06-23
...
> מבוצע לפי `PROMPTER.md`.

**מומש 1:1:**

| פריט | מה תוקן | קובץ | בדיקה |
|---|---|---|---|
| 3 | `displayBackgroundShape` — רקע `w:background` מצויר רק כשהדגל קיים (over‑display תוקן) — **נסגר במשימה 14** | `docx_reader.dart` | `settings_parsing_test.dart` |

**נותר נדחה / סטיות מודעות:**

- **סימן מים טקסטואלי `v:textpath` (4, הקריטי ביותר):** נופל ל‑`DocxRawInline` → נעלם; דורש רינדור VML textpath — נדחה (גדול).
- **סימן מים בכותרת מאחורי הגוף (5,7), `v:background`→`v:fill` תקני (2), `themeColor` של רקע (1, תלוי §13 chrome), סיבוב/אטימות במסלול הרקע (6,7):** נותרו פערים מתועדים — נדחים.
