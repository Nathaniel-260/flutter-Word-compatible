...
>
> **סטטוס סריקה:** ✅ נסקר במלואו &nbsp;|&nbsp; **סטטוס מימוש:** 🔄 ruby base (42) מומש; ptab/OMML/comments נותרו נדחים &nbsp;|&nbsp; **עודכן לאחרונה:** 2026-06-23
...
|---|---|---|---|
| 42 | `w:ruby` — טקסט הבסיס (`w:rubyBase`) נשמר ומרונדר עם עיצוב הריצה, במקום אובדן מוחלט ל‑`DocxRawInline` | `inline_parser.dart` (`parseRun`) | `inline_extras_test.dart` |

**נותר נדחה / סטיות מודעות:**

- **`w:ptab` (8):** נקרא אך לא מרונדר — דורש חיווט ל‑TabEngine + מדידה; נדחה.
- **תווית `w:rt` של ruby (מעל הבסיס):** לא מרונדרת — מגבלה מתועדת.
- **`tab` בלי stops=4 רווחים (4):** סטייה מודעת (כמו משימה 04 §29; שמירת measure≡render).
- **סגנונות Hyperlink/FootnoteReference קשיחים (31,34,35), separator מותאם (36), `pos`/`numStart` הערות (37), PAGEREF \h (17), OMML מלא (43–45), comments (39–41):** נותרו פערים מתועדים / משימות נפרדות.
