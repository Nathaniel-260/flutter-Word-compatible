...
>
> **סטטוס סריקה:** ✅ נסקר במלואו &nbsp;|&nbsp; **סטטוס מימוש:** 🟨 פערים בני‑מימוש נסגרו 1:1 (smallCaps→smcp אמיתי; דגלי `w:rtl`/`w:cs`→כפיית כתב‑מורכב לקטע ניטרלי; `w:u`=`words`→קו רק מתחת מילים) + אומת ש‑auto‑color ו‑shd‑ריצה כבר נסגרו ב‑task 02; שאר הפערים = סטיות מודעות מנומקות (position/w/fitText/em — אילוץ מבני ב‑Flutter; w14/EA/דגלים נדירים) — כל פער עם הכרעה ב‑ב.2 &nbsp;|&nbsp; **עודכן לאחרונה:** 2026-06-22
...
> **אימות שבוצע (2026-06-22):** `flutter analyze` — viewer נקי; docx_creator עם 3 info‑lints קיימים‑מראש (`unnecessary_import`) בקבצי‑בדיקה שלא נגעתי בהם (`field_parsing_test.dart`, `section_page_numbering_test.dart`). `flutter test` — docx_creator **461✅** (1 דילוג, +3 בדיקות חדשות), docx_file_viewer **393✅** (4 הנכשלות = קובצי‑fixture חסרים בצ'קאאוט ב‑`hebrew_rtl_golden_test.dart` בלבד, `PathNotFoundException`, לא קוד). בדיקות חדשות: `inline_extras_test.dart` (3 — קריאה/off/round‑trip ל‑`w:cs`, עברית+אנגלית), `script_split_test.dart` (8 — smallCaps→smcp + rtl/cs→כתב‑מורכב + `w:u`=words, עברית+אנגלית, כולל בדיקת מדידה≡רינדור), `highlight_palette_test.dart` (2 — 16 צבעי ST_HighlightColor המדויקים).
...
- **`w:dstrike`+underline (פריט 15).** *חומרה: נמוכה.* Flutter חולק `decorationStyle` יחיד לכל קישוטי הקטע, כך שכאשר יש גם קו תחתון, הכפילות של הקו‑החוצה אובדת (מצויר יחיד). צירוף נדיר; ציור ידני דורש overlay/CustomPaint.
...
- **mark run properties — rPr בתוך pPr (פריט 1).** *חומרה: נמוכה.* עיצוב סימן הפסקה (גודל גופן של פיד הפסקה) משפיע בעיקר על **גובה פסקה ריקה**. **נחקר ונדחה מודעת (2026-06-23):** הקריאה זמינה כמעט בחינם — `block_parser` כבר מעביר את ה‑rPr של ה‑pPr ל‑`DocxStyle.fromXml`, כך ש‑`parsedProps.fontSize` הוא גודל סימן‑הפסקה הישיר. **החסם האמיתי ברינדור:** ה‑renderer מצמצם פסקה **ריקה** ל‑`SizedBox()` בגובה 0 (הגובה מגיע רק מ‑spacing/שטח‑שמור), בעוד ה‑`TextMeasurer` סופר לה שורה מלאה (ZWSP) — **פער measure≠render קיים‑מראש במודל הפסקה‑הריקה**, בלתי‑תלוי ב‑rPr. מימוש גודל‑מתוך‑הסימן דורש קודם לפשר את שני המודלים (לרנדר פסקה ריקה כשורה אמיתית) — שינוי בעל **רדיוס פגיעה רחב** (כל פסקה ריקה בכל מסמך, וגם הבדל paged↔continuous), שאינו תוספת מוכלת של rPr. שינוי ה‑measurer לבדו (שניסיתי) מזיז עימוד ללא תועלת ויזואלית — לכן בוטל. **ל‑AI הבא:** משימה עצמאית — לאחד את מודל הפסקה‑הריקה (renderer↔measurer) ואז להזין `markRunFontSize` לשני הנתיבים.
