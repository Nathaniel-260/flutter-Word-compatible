...
> **אימות שבוצע (2026-06-22):** `flutter analyze` — viewer נקי; docx_creator עם 3 info‑lints קיימים‑מראש (`unnecessary_import`) בקבצי‑בדיקה שלא נגעתי בהם (`field_parsing_test.dart`, `section_page_numbering_test.dart`). `flutter test` — docx_creator **461✅** (1 דילוג, +3 בדיקות חדשות), docx_file_viewer **391✅** (4 הנכשלות = קובצי‑fixture חסרים בצ'קאאוט ב‑`hebrew_rtl_golden_test.dart` בלבד, `PathNotFoundException`, לא קוד). בדיקות חדשות: `inline_extras_test.dart` (3 — קריאה/off/round‑trip ל‑`w:cs`, עברית+אנגלית), `script_split_test.dart` (8 — smallCaps→smcp + rtl/cs→כתב‑מורכב + `w:u`=words, עברית+אנגלית, כולל בדיקת מדידה≡רינדור).
...
- **`w:highlight` — פלטת Material (פריט 28).** *חומרה: נמוכה.* 17 הערכים ממופים לצבעי Material (קירוב גוון), לא לפלטת ה‑highlight המדויקת של Word.
...
- **mark run properties — rPr בתוך pPr (פריט 1).** *חומרה: נמוכה.* עיצוב סימן הפסקה (גודל גופן של פיד הפסקה) לא נקרא; משפיע בעיקר על **גובה פסקה ריקה**. **ל‑AI הבא:** לקרוא ב‑`_parseParagraphProperties` ולהשחיל גובה‑שורה נומינלי לפסקה ריקה בשני נתיבי measure+render.
