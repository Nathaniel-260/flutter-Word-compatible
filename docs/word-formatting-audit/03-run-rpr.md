...
>
> **סטטוס סריקה:** ✅ נסקר במלואו &nbsp;|&nbsp; **סטטוס מימוש:** 🟨 פערים בני‑מימוש נסגרו 1:1 (smallCaps→smcp אמיתי; דגלי `w:rtl`/`w:cs`→כתב‑מורכב לקטע ניטרלי; `w:u`=`words`→קו רק מתחת מילים; `w:highlight`→פלטה מדויקת; mark‑size→גובה פסקה ריקה + תיקון קיפול‑ל‑0; dstrike+underline→פשרת "הכפול גובר"; w:bdr→כיבוד w:space + מידול תיבת‑מסגרת במדידה) + אומת ש‑auto‑color ו‑shd‑ריצה כבר נסגרו ב‑task 02; שאר הפערים = סטיות מודעות מנומקות (position/w/fitText/em — אילוץ מבני ב‑Flutter; w14/EA/דגלים נדירים) — כל פער עם הכרעה ב‑ב.2 &nbsp;|&nbsp; **עודכן לאחרונה:** 2026-06-22
...
| 37 | `w:bdr` (מסגרת ריצה + מיזוג רצף) | כן | נאמן~ | נקרא (`_parseBorderSide`→`textBorder`) ומרונדר כ‑`Container`/`Border.all`. ✅ **תוקן:** (א) **`w:space` מכובד** (pt→px כ‑padding אופקי); (ג) מרונדר **תמיד** כולל בזמן חיפוש (ההדגשה מושחלת לתוך הקופסה דרך `_overlaySegment`); (ד) ה‑**measurer ממדל את תיבת המסגרת** כ‑placeholder בגודל זהה (גאומטריה משותפת `textBorderBox`) → מדידה≡רינדור (נבדק). הריצה הממוסגרת אטומית (שורה אחת). *שאריות:* (ב) אין מיזוג רצף ריצות; ריצה רחבה מהשורה נשארת שורה‑אחת (נדיר). | `span_factory.dart` (`textBorderBox`+measurer); `paragraph_builder.dart` |
...
> **אימות שבוצע (2026-06-22):** `flutter analyze` — viewer נקי; docx_creator עם 3 info‑lints קיימים‑מראש (`unnecessary_import`) בקבצי‑בדיקה שלא נגעתי בהם (`field_parsing_test.dart`, `section_page_numbering_test.dart`). `flutter test` — docx_creator **464✅** (1 דילוג, +6 בדיקות חדשות), docx_file_viewer **402✅** (4 הנכשלות = קובצי‑fixture חסרים בצ'קאאוט ב‑`hebrew_rtl_golden_test.dart` בלבד, `PathNotFoundException`, לא קוד). בדיקות חדשות: `inline_extras_test.dart` (3 — `w:cs`), `paragraph_properties_test.dart` (3 — `markRunFontSize`), `script_split_test.dart` (11 — smallCaps→smcp + rtl/cs + `w:u`=words + dstrike+underline), `highlight_palette_test.dart` (2 — ST_HighlightColor), `empty_paragraph_height_test.dart` (3 — גובה פסקה ריקה), `text_border_test.dart` (3 — `w:bdr` גאומטריה+אטומי+measure≡render). כל בדיקות הטקסט כוללות עברית+אנגלית.
...
- **`w:bdr` — מסגרת ריצה (פריט 37) — מומש (עם שאריות).** *חומרה: נמוכה.* ✅ `w:space` מכובד (pt→px), המסגרת מרונדרת **תמיד** (כולל בחיפוש — ההדגשה מושחלת לקופסה דרך `_overlaySegment`), וה‑measurer ממדל את תיבת המסגרת כ‑`WidgetSpan` placeholder בגודל זהה לזה של ה‑`Container` ברינדור, דרך גאומטריה **משותפת** (`SpanFactory.textBorderBox`: padding מ‑`w:space`, עובי קו מ‑`w:sz/8`) → מדידה≡רינדור (נבדק ב‑`text_border_test.dart`). הריצה הממוסגרת אטומית (placeholder יחיד, שורה אחת). *שאריות מודעות:* (ב) **אין מיזוג** רצף ריצות בעלות אותו bdr (כל ריצה ממוסגרת בנפרד — דורש grouping פר‑bdr); וריצה ממוסגרת **רחבה מהשורה** נשארת שורה‑אחת (Word עוטף את המסגרת על מספר שורות — נדיר, דורש מסלול עטיפה ייעודי).
