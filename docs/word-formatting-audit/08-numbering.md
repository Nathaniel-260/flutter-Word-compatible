...
>
> **סטטוס סריקה:** ✅ נסקר במלואו &nbsp;|&nbsp; **סטטוס מימוש:** 🔄 numId=0 (31) + rPr תווית (22) מומשו; מודל הזחת‑רמה נדחה &nbsp;|&nbsp; **עודכן לאחרונה:** 2026-06-23
...
> מבוצע לפי `PROMPTER.md`. בדיקות נלוות; `flutter analyze` נקי; סוויטות המספור ירוקות.

**מומש 1:1:**

| פריט | מה תוקן | קובץ | בדיקה |
|---|---|---|---|
| 31 | `numId="0"` מבטל מספור (פסקה רגילה, לא תבליט שקרי) — **נסגר במשימה 04** | `block_parser.dart` | `paragraph_ppr_04_test.dart` |
| 22 | rPr של התווית — `color w:val`/`sz`/`b`/`i` נקראים ל‑`DocxNumberingLevel` ומעצבים את ה‑marker (color/size/bold כבר נצרכים ב‑`list_builder`) | `numbering_parser.dart`, `docx_theme.dart`, `block_parser.dart` | `numbering_level_extras_test.dart` |

**נותר נדחה / סטיות מודעות:**

- **מודל הזחת‑רמה (21) + `lvlJc` (20) + `suff=tab` (16):** `list_builder` מחשב הזחה בנוסחה פר‑רמה במקום `left`/`hanging` המוחלטים — ריפקטור פיקסלי תלוי‑כיוון; נדחה (גדול/מסוכן).
- **`numStyleLink`/`styleLink` (5,6), `lvlOverride` עם `lvl` מלא (27), `pStyle` ברמה (14):** דורשים חיווט numbering↔styles — נדחה.
- **פורמטי numFmt לא‑עבריים (12), גודל תבליט‑תמונה (18,28), `italic` של תווית:** סטיות מודעות (עברית hebrew1/2 נאמנה — מוקד הפרויקט מכוסה).
